			...
			<div class="sku-box store-content">
				...
				<div class="gray-box">
					<div class="item-box">
                        ...
                        <!-- 调用GoodItem组件的时候传递两个数据包：
                        1，gitem用以渲染详情数据
                        2，gidx 传递数据包下标 -->
						<GoodItem 
                        v-for="(item,index) in gdata" 
                        :key="index" 
                        :gitem="item" 
                        :gidx="index"
                        />
...
export default {
    ...
    methods: {
        scrollFn(){
            let  screenH = window.outerHeight; //获取屏幕高度
            let H = this.$refs.loading.offsetTop; //loading元素距离文档最顶部的距离
            let sT = document.documentElement.scrollTop; //滚动距离
            if(sT+screenH>=H){
                console.log('开始加载')
