					...
					<div class="gallery-wrapper">
						<div class="gallery">
							<div class="thumbnail">
								<ul>
									<li 
                                     v-for="(item,index) in detailD.stock_info[cNum].imgs" 
                                     :key="index"
                                     :class="index==iNum?'on':''" 
                                     @click="iNum=index">
                                        <img :src="item">
							...
							<div class="thumb">
								<ul>
									<li class="on">
                                        <img :src="detailD.stock_info[cNum].imgs[iNum]">
                                    </li>
								</ul>
							</div>
						</div>
					...
					<div class="banner">
						<div class="sku-custom-title">
							<div class="params-price">
								<span><em>¥</em><i>{{detailD.stock_info[cNum].price}}</i></span>
							</div>
							<div class="params-info">
								<h4>{{detailD.stock_info[cNum].title}}</h4>
								<h6>{{detailD.stock_info[cNum].sub_title}}</h6>
							</div>
						</div>
						<div class="sku-dynamic-params-panel">
							<div class="sku-dynamic-params clear">
								...
								<ul class="params-colors">
									<li 
                                    v-for="(item,index) in detailD.stock_info" 
                                    :key="index" 
                                    :class="index==cNum?'cur':''" 
                                    @click="cNum=index">
...
<script>
import goodsData from '@/assets/js/goodsData.js'
export default {
    data() {
        return {
            detailD: {}, //渲染详情的数据包
    ...
    created() {
        let idx = this.$route.params.idx;  //【day02-三-3】提取路由参数
        this.detailD=goodsData[idx]  //根据路由参数所的提供下标，提取一个商品数据包渲染详情页面
    },
}
</script>
